...
/**
 * Default JSON wire serializer for MessageEnvelope.
 *
 * Wire format: {"id": ..., "type": ..., "payload": ..., "headers": {...}}.
 * serialize() requires a JSON-safe payload (null, scalar, arrays and nested
 * combinations; resources and objects are rejected). deserialize() accepts at
 * most 1 MiB of input and defaults a missing "payload" key to null.
 */
final class JsonMessageSerializer implements MessageSerializerInterface
{
    ...
    public function deserialize(string $payload): MessageEnvelope
    {
        ...
        if (!is_array($data) || !isset($data['id'], $data['type']) || !is_string($data['id']) || !is_string($data['type'])) {
