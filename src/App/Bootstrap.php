...
namespace Zef\App {
    ...
    {
        public static function createApp(bool $debug = false, ?\Psr\Log\LoggerInterface $logger = null, ?int $maxBodyBytes = null): Application
        {
            if ($maxBodyBytes === null) {
                $rawLimit = getenv('ZEF_MAX_BODY_BYTES');
                $maxBodyBytes = ($rawLimit !== false && ctype_digit((string) $rawLimit)) ? (int) $rawLimit : null;
            }
            $app = new Application($debug, $logger, $maxBodyBytes === null ? null : new \Zef\Framework\Http\RequestBodyPolicy($maxBodyBytes));
            $trusted = getenv('ZEF_TRUSTED_HOSTS');
            $hosts = $trusted !== false && trim($trusted) !== '' ? array_map('trim', explode(',', (string) $trusted)) : ['localhost','127.0.0.1','::1','zef.test'];
