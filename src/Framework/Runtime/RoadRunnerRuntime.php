...
namespace Zef\Framework\Runtime {
    ...
    /** Coordinates one single-use application lifecycle over a worker transport. */
    final class RoadRunnerRuntime implements RuntimeInterface
    {
        ...
        /** @var array{state:string,instance_id:string,worker_id:string,started_at_ns:int}|null */
        private ?array $lifecycle = null;
        /** @var array{resource_capacity:int,saturation_percent:int,control_plane_enabled:bool,distributed_compatibility:bool} */
        private array $runtimeConfig = ['resource_capacity' => 1,'saturation_percent' => 90,'control_plane_enabled' => false,'distributed_compatibility' => true];
        /** @var array{admitted:int,completed:int,rejected:int,dropped:int,saturation:int} */
        private array $resourceCounters = ['admitted' => 0,'completed' => 0,'rejected' => 0,'dropped' => 0,'saturation' => 0];
        ...
        #[\Override]
        /** @return int Zero on normal exhaustion/shutdown, non-zero on worker failure or memory guard. */
            ...
            try {
                while (!$this->stopRequested && $this->worker->isRunning()) {
                    ...
                    try {
                        ...
                        if ($this->application->isBooted() && $this->handled === 1 && !$this->stopRequested) {
                            $this->transitionLifecycle('ready');
                            $this->recordLifecycle('worker.ready');
                        }
                        $this->worker->respond($response);
                        try {
                            $telemetry = $this->application->getContainer()->get(\Zef\Framework\Observability\Telemetry::class);
                            if ($telemetry instanceof \Zef\Framework\Observability\Telemetry) {
                                $telemetry->flush();
                            }
                        } catch (\Throwable) {
                            // Telemetry delivery is strictly non-critical to request correctness.
                        }
                    } catch (\Throwable $e) {
                        $this->reportWorkerFailure($e);
        ...
        #[\Override]
        /** Request a graceful drain of the current worker loop. */
        public function stop(): void
        {
            ...
            try {
                $this->worker->stop();
            } catch (\Throwable) { /* supervisor remains authoritative */
            }
        }
        #[\Override]
        /** Report whether the runtime loop is active. */
        public function isRunning(): bool
        {
            return $this->running;
        }
        /** Return the number of requests admitted for application handling. */
        ...
        /** @return array{resource_capacity:int,saturation_percent:int,control_plane_enabled:bool,distributed_compatibility:bool} */
        private function loadRuntimeConfig(): array
        {
            ...
            $env = static function (string $name, mixed $default): mixed {
                $raw = getenv($name);
                return $raw === false || trim($raw) === '' ? $default : $raw;
            };
            $capacity = filter_var($env('ZEF_RUNTIME_RESOURCE_CAPACITY', $defaults['resource_capacity']), FILTER_VALIDATE_INT);
            $saturation = filter_var($env('ZEF_RUNTIME_SATURATION_PERCENT', $defaults['saturation_percent']), FILTER_VALIDATE_INT);
            if ($capacity === false || $capacity < 1 || $capacity > 1024) {
                throw new \InvalidArgumentException('Invalid ZEF_RUNTIME_RESOURCE_CAPACITY.');
            }
            if ($saturation === false || $saturation < 50 || $saturation > 99) {
                throw new \InvalidArgumentException('Invalid ZEF_RUNTIME_SATURATION_PERCENT.');
            }
            $controlRaw = $env('ZEF_RUNTIME_CONTROL_PLANE', 'off');
            $control = is_string($controlRaw) ? strtolower(trim($controlRaw)) : 'off';
            if (!in_array($control, ['on','off'], true)) {
                throw new \InvalidArgumentException('Invalid ZEF_RUNTIME_CONTROL_PLANE.');
            }
            return [
                'resource_capacity' => (int) $capacity,
                'saturation_percent' => (int) $saturation,
                'control_plane_enabled' => $control === 'on',
                'distributed_compatibility' => true,
            ];
        }
        ...
        private function initializeLifecycle(): void
        {
            ...
            $this->lifecycle = ['state' => 'starting','instance_id' => $instance,'worker_id' => $worker,'started_at_ns' => hrtime(true)];
            $boundary = $this->compatibilityBoundary();
            if ($boundary['enabled']) {
                $this->recordLifecycle('runtime.compatibility.boundary.ready');
            }
            if ((bool) $this->runtimeConfig['control_plane_enabled'] && !$this->validateControlCommand('lifecycle.status')) {
        ...
        private function transitionLifecycle(string $next): void
        {
            ...
            $allowed = ['starting' => ['ready','draining','stopped'],'ready' => ['draining','stopped'],'draining' => ['stopped'],'stopped' => []];
            if ($current === $next) {
                return;
            }
        ...
        private function recordResource(string $event, array $attributes = []): void
        {
            try {
                $telemetry = $this->application->getContainer()->get(\Zef\Framework\Observability\Telemetry::class);
                if ($telemetry instanceof \Zef\Framework\Observability\Telemetry) {
                    $telemetry->recordLog('INFO', 'runtime.resource.' . $event, array_merge(['event.name' => 'runtime.resource.' . $event], $attributes));
                    $telemetry->meter()->increment('zef.runtime.resource.events.total', 1, ['event.name' => $event]);
                }
            } catch (\Throwable) { /* telemetry remains fail-open */
            }
        }
        ...
        private function recordLifecycle(string $event): void
        {
            try {
                ...
                // Lifecycle telemetry is strictly non-critical.
            }
        }
        ...
        {
            if (!$this->installSignalHandlers || $this->signalsInstalled || !function_exists('pcntl_signal') || !function_exists('pcntl_async_signals')) {
                return;
            }
        ...
        /**
         * Restore process-default handling for signals installed by this runtime instance.
         *
         * RoadRunner owns process supervision; Zef only installs temporary termination hooks
         * so a runtime can drain/stop cleanly. Removing those hooks prevents a terminated
         * Runtime object from being retained by PHP's process-global signal table.
         */
