...
namespace Zef\Framework\Observability {
    final class Span implements SpanInterface
    {
        ...
        /** @var list<array{name:string,time_unix_nano:int,attributes:array<string,mixed>}> */ private array $events = [];
        ...
        #[\Override] public function addEvent(string $name, array $attributes = []): self
        {
            ...
            $this->events[] = ['name' => $name, 'time_unix_nano' => TelemetryClock::nowUnixNano(), 'attributes' => $clean];
            return $this;
        }
        #[\Override] public function setStatus(string $status, ?string $description = null): self
        {
            if (!$this->ended) {
                ...
                $this->status = $status;
                $this->statusDescription = $description === null ? null : TelemetrySanitizer::string($description, 1024);
            }
            return $this;
        }
        #[\Override] public function end(?int $endNs = null): void
        {
            ...
            ($this->onEnd)(new SpanData($this->name, $this->context, $this->parent, $this->startNs, $this->endNs, $this->startUnixNano, $this->startUnixNano + (int) (($this->endNs - $this->startNs)), $this->status, $this->statusDescription, $this->attributes, $this->events));
        }
