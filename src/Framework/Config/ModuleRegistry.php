    ...
    final class ModuleRegistry
    {
        ...
        public function resolveOrder(): array
        {
            ...
            $visit = function (string $key) use (&$visit, &$state, &$ordered): void {
                ...
                $module = $this->modules[$key] ?? throw new InvalidConfigurationException("Missing module dependency '{$key}'.");
                $state[$key] = 1;
        ...
        public function registerAll(ModuleRegistrar $registrar, \Psr\Container\ContainerInterface $container): void
        {
            if ($this->registered) {
                return;
            }
        ...
        public function bootAll(\Psr\Container\ContainerInterface $container): void
        {
            if (!$this->registered) {
        ...
        public function startAll(\Psr\Container\ContainerInterface $container): void
        {
            if (!$this->booted) {
        ...
        public function shutdownAll(\Psr\Container\ContainerInterface $container): void
        {
            if (!$this->registered) {
                return;
            }
